...
interface AssistedPasswordProps {
  password: string
}

export function AssistedPasswordConfirmation({ password }: AssistedPasswordProps) {
  ...
  const bounceAnimation = { x: shake ? [-10, 10, -10, 10, 0] : 0, transition: { duration: 0.5 } }
  const matchAnimation = { scale: passwordsMatch ? [1, 1.05, 1] : 1, transition: { duration: 0.3 } }
  const borderAnimation = { borderColor: passwordsMatch ? "#10B981" : "", transition: { duration: 0.3 } }

  return (
    <main className="relative flex min-h-screen w-full items-start justify-center px-4 py-10 md:items-center">
      <div className="z-10 flex w-full flex-col items-center">
        <div className="mx-auto flex h-full w-full max-w-lg flex-col items-center justify-center gap-8 bg-white p-8 sm:p-12 rounded-2xl shadow-xl">
          <div className="relative flex w-full flex-col items-start justify-center">
            ...
            <motion.div className="mb-3 mt-1 h-[52px] w-full rounded-xl border-2 bg-white px-2 py-2" animate={{ ...bounceAnimation, ...matchAnimation, ...borderAnimation }}>
                ...
                <div className="z-10 flex h-full items-center justify-center bg-transparent px-0 py-1 tracking-[0.15em]">
                  {password.split("").map((_, index) => (
                    <div key={index} className="flex h-full w-4 shrink-0 items-center justify-center">
                      <span className="size-[5px] rounded-full bg-black" />
                    </div>
                  ))}
                </div>
                <div className="absolute bottom-0 left-0 top-0 z-0 flex h-full w-full items-center justify-start">
                  {password.split("").map((letter, index) => (
                    <motion.div
                      ...
                      style={{ left: `${index * 16}px`, scaleX: confirmPassword[index] ? 1 : 0, transformOrigin: "left" }}
                    />
                  ))}
                </div>
              </div>
...
export const AssistedPasswordConfirmationDemo = () => <AssistedPasswordConfirmation password="ui.lndev.me" />
