...
export function SidebarDemo() {
  const links = [
    { label: "Dashboard", href: "#", icon: <span className="h-5 w-5 rounded-full bg-neutral-700 dark:bg-neutral-200" /> },
    { label: "Perfil", href: "#", icon: <span className="h-5 w-5 rounded-full bg-neutral-700 dark:bg-neutral-200" /> },
    { label: "Ajustes", href: "#", icon: <span className="h-5 w-5 rounded-full bg-neutral-700 dark:bg-neutral-200" /> },
  ]
  const [open, setOpen] = useState(false)
  return (
    <div className={cn("rounded-md flex flex-col md:flex-row bg-gray-100 dark:bg-neutral-800 w-full flex-1 max-w-5xl mx-auto border border-neutral-200 dark:border-neutral-700 overflow-hidden", "h-[60vh]")}>
      <Sidebar open={open} setOpen={setOpen}>
        <SidebarBody className="justify-between gap-10">
          <div className="flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
            <div className="h-8 w-8 bg-neutral-900 dark:bg-white rounded-lg" />
          ...
          <div>
            <SidebarLink link={{ label: "Usuario", href: "#", icon: <div className="h-7 w-7 rounded-full bg-neutral-900 dark:bg-white" /> }} />
          </div>
        </SidebarBody>
      </Sidebar>
      <div className="flex flex-1">
        <div className="p-4 md:p-8 rounded-tl-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 flex flex-col gap-2 flex-1 w-full h-full">
          <div className="flex gap-2">
            {[...new Array(4)].map((_, i) => (
              <div key={i} className="h-16 w-full rounded-lg bg-gray-100 dark:bg-neutral-800" />
            ))}
          </div>
        </div>
      </div>
